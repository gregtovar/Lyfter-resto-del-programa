...
class NumberArray:
    ...
    # Adds one value at the end of the chain.
    ...
    def draw_chart(self, left: int, right: int, locked_from: int) -> None:
        ...
        while walker is not None:
            ...
            elif locked_from >= 0 and index >= locked_from:
    ...
    def draw_grid(self, left: int, right: int, locked_from: int) -> None:
        walker = self.head
        index = 0
        line = "   "
    ...
    def draw_frame(self, left: int, right: int, locked_from: int,
                   heading: str) -> None:
        cursor_home()
        draw_header("")
        ...
        self.draw_chart(left, right, locked_from)
        print("\033[K")
        self.draw_grid(left, right, locked_from)

        print("\033[K")
        print(paint("   red = being swapped   ·   green = locked in place   "
    ...
    # Runs bubble sort, redrawing the array on every swap.
    ...
    def bubble_sort(self, delay: float) -> bool:
        ...
        try:
            # Each loop is one pass over the still unsorted part of the chain.
            while swapped_any:
                swapped_any = False
                ...
                while index < limit:
                    ...
                    if walker.value > walker.next.value:
                        ...
                        walker.next.value = temp

                        self.swaps = self.swaps + 1
                        swapped_any = True

                        if delay > 0:
                            self.draw_frame(index, index + 1,
                                            self.size - locked,
                                            "SWAPPING position " + str(index + 1)
                                            + " with position " + str(index + 2))
                            time.sleep(delay)
                ...
                if delay == 0:
                    self.draw_frame(-1, -1, self.size - locked,
...
def option_sort(array: "NumberArray") -> None:
    ...
    print()
    array.draw_chart(-1, -1, 0)
    print()
    array.draw_grid(-1, -1, 0)
