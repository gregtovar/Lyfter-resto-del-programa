...
#
#  Function: count_by_column - Reads the CSV, shows the unique values found in the given column, asks the user which one to count, prints the count, and shows the matching records as a fancy table.
#
def count_by_column(file_path: str, column_name: str) -> None:
    rows = read_csv_rows(file_path)
    ...
    # Find the target column by name (case-insensitive).
    ...
    if target_col_index is None:
        print(f"\n{C['RED']}Column '{column_name}' was not found in the file.{C['RESET']}")
        return

    display_col_name = headers[target_col_index][:1].upper() + headers[target_col_index][1:]

    # Collect unique values found in the target column.
    ...
    if not unique_values:
        print(f"\n{C['RED']}No values were found in the '{display_col_name}' column.{C['RESET']}")
        return

    # Report the available values to the user.
    print(f"\n{C['DARK_GREEN']}=== Count by {display_col_name} ==={C['RESET']}")
    print(f"{C['BLUE']}Available values in '{display_col_name}': {C['RESET']}"
    ...
    count = sum(
        ...
        if target_col_index < len(row) and row[target_col_index].strip().upper() == chosen_upper
    )

    display_value = seen[chosen_upper]
    print(f"\n{C['GREEN']}Total records with {display_col_name} = '{display_value}': "
          f"{count}{C['RESET']}")
    ...
    if filtered_rows:
        render_fancy_table(
            field_names,
            filtered_rows,
            f"Records with {display_col_name} = '{display_value}'",
        )
...
def main():
    ...
    count_by_column(file_path, "Clasificacion")
    count_by_column(file_path, "Desarrollador")
